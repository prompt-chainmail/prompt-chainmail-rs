//! Tool-use-hijacking family rivet (ONNX classifier-backed).
...
impl Rivet for ToolUseHijackingRivet {
    ...
    fn process(
        ...
        let result = self.classifier.classify_family(
            &context.sanitized,
            &primary_language,
            ClassifierFamily::ToolUseHijacking,
        ...
        if is_attack {
            context
                .flags
                .insert(security_flags::TOOL_USE_HIJACKING.to_string());
            ...
            context
                .metadata
                .insert("tool_use_hijacking_detected".to_string(), json!(true));
            context.metadata.insert(
                "tool_use_hijacking_attack_types".to_string(),
                json!(result.attack_types),
            );
        } else {
            context
                .metadata
                .insert("tool_use_hijacking_detected".to_string(), json!(false));
            context
                .metadata
                .insert("tool_use_hijacking_attack_types".to_string(), json!([]));
        }

        context.metadata.insert(
            "tool_use_hijacking_confidence".to_string(),
            json!(max_confidence),
        );
        context.metadata.insert(
            "tool_use_hijacking_risk_score".to_string(),
            json!(max_risk_score),
        );
        context.metadata.insert(
            "tool_use_hijacking_detected_language".to_string(),
            json!(primary_language),
        );
        context.metadata.insert(
            "tool_use_hijacking_detected_languages".to_string(),
            json!(top_languages
                .iter()
        ...
        context.metadata.insert(
            "tool_use_hijacking_matches".to_string(),
        ...
        if let Some(detector_error) = result.detector_error {
            ...
            context.metadata.insert(
                "tool_use_hijacking_detector_error".to_string(),
                json!(detector_error),
            );
        }
